...
if (isset($_GET['idevent'])) {
    ...
    $teamsInEvent = $obj->getTeamsInEvent($idevent); 
...
<body>
    ...
    <form action="updateevent_proses.php" method="POST">
        ...
        <label for="idteam">Choose Teams:</label><br>
        <?php foreach ($team as $teams): ?>
            <input type="checkbox" name="team[]" value="<?php echo $teams['idteam']; ?>"
                <?php echo in_array($teams['idteam'], $teamsInEvent) ? 'checked' : ''; ?>> 
            <?php echo $teams['name']; ?><br>
        <?php endforeach; ?>
        <br><br>
